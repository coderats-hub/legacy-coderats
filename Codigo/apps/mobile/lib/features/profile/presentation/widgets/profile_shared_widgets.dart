...
/// Cabeçalho de perfil com imagem, nome e botão de ação (ex: "Ver GitHub")
...
/// Botão de ação em forma de chip colorido (ex: "Adicionar GitHub")
...
/// Cartão de calendário reutilizável (usa TableCalendar)
...
/// Seção de "badges" do perfil
class BadgesRow extends StatelessWidget {
  final bool showSeeAll;

  const BadgesRow({super.key, this.showSeeAll = false});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const Row(
          children: [
            BadgePlaceholder(label: '2x'),
            SizedBox(width: 12),
            BadgePlaceholder(label: '1x', warning: true),
          ],
        ),
      ],
    );
...
/// Representa um ícone simples de badge (placeholder visual)
...
/// Seção "Grupos em Comum"
class GroupsInCommon extends StatelessWidget {
  const GroupsInCommon({super.key});
